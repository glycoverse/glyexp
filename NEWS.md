...
* The legacy `experiment()` container constructor, conversion helpers, accessors, metadata helpers, dimension helpers, and S3 methods are now deprecated. Use `GlycomicSE()` or `GlycoproteomicSE()` as the default data container. The dplyr-style manipulation functions remain available.
