...
#'
#' @details
#' The `glycan_structures` attribute will be a named list 
#' of glycan graph objects.
...
#' @param override A logical value.
#'   If `TRUE`, override existing glycan structures.
...
add_structures <- function(exp, mode = "ne", override = FALSE, silence = FALSE) {
  ...
  parser <- switch(
    ...
    pglyco = glyparse::parse_pglyco_struc,
    strucgp = glyparse::parse_strucgp_struc,
    msfragger = glyparse::parse_wurcs,
