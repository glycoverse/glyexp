...
glycomics experimental data. `GlycomicSE()` and `GlycoproteomicSE()` validate
glycan-specific annotations while preserving the standard assay, `rowData`,
`colData`, and metadata interfaces.

The legacy `experiment()` API and its related helpers are deprecated but remain
available for a staged migration of the `glycoverse` ecosystem.
...
`GlycomicSE()` and `GlycoproteomicSE()` are the recommended data containers for
new workflows. They are `SummarizedExperiment` subclasses, so other packages
can use standard Bioconductor assay and annotation interfaces directly.
