test_that("mutating sample info works", {
  ...
  new_exp <- mutate_samples(exp, new_col = 1:3)
...
test_that("mutating variable info works", {
  ...
  new_exp <- mutate_variables(exp, new_col = 1:3)
...
test_that("mutating using non-existing columns raises an error", {
  ...
  expect_snapshot(mutate_samples(exp, new_col = bad_col), error = TRUE)
  expect_snapshot(mutate_variables(exp, new_col = bad_col), error = TRUE)
})
...
test_that("trying to mutate 'sample' with duplicated values raises an error", {
  ...
  expect_snapshot(mutate_samples(exp, sample = 1), error = TRUE)
})
...
test_that("trying to mutate 'variable' with duplicated values raises an error", {
  ...
  expect_snapshot(mutate_variables(exp, variable = 1), error = TRUE)
})
...
test_that("mutating 'sample' updates 'expr_mat' correctly", {
  ...
  new_exp <- mutate_samples(exp, sample = c("A", "B", "C"))
...
test_that("mutating 'variable' updates 'expr_mat' correctly", {
  ...
  new_exp <- mutate_variables(exp, variable = c("A", "B", "C"))
