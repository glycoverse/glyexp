test_that("slice_samples works", {
  exp <- create_test_exp(c("S1", "S2", "S3", "S4", "S5"), c("V1", "V2", "V3"))
  
  sliced_exp <- slice_samples(exp, 1, 3, 5)
...
test_that("slice_variables works", {
  exp <- create_test_exp(c("S1", "S2"), c("V1", "V2", "V3", "V4", "V5"))
  
  sliced_exp <- slice_variables(exp, 2, 4)
...
test_that("slice_head_samples works", {
  exp <- create_test_exp(c("S1", "S2", "S3", "S4", "S5"), c("V1", "V2"))
  
  sliced_exp <- slice_head_samples(exp, n = 3)
...
test_that("slice_head_variables works", {
  exp <- create_test_exp(c("S1", "S2"), c("V1", "V2", "V3", "V4"))
  
  sliced_exp <- slice_head_variables(exp, n = 2)
...
test_that("slice_tail_samples works", {
  exp <- create_test_exp(c("S1", "S2", "S3", "S4", "S5"), c("V1", "V2"))
  
  sliced_exp <- slice_tail_samples(exp, n = 2)
...
test_that("slice_tail_variables works", {
  exp <- create_test_exp(c("S1", "S2"), c("V1", "V2", "V3", "V4"))
  
  sliced_exp <- slice_tail_variables(exp, n = 2)
...
test_that("slice_sample_samples works", {
  set.seed(123)
  exp <- create_test_exp(c("S1", "S2", "S3", "S4", "S5"), c("V1", "V2"))
  
  sliced_exp <- slice_sample_samples(exp, n = 3)
...
test_that("slice_sample_variables works", {
  set.seed(123)
  exp <- create_test_exp(c("S1", "S2"), c("V1", "V2", "V3", "V4", "V5"))
  
  sliced_exp <- slice_sample_variables(exp, n = 3)
...
test_that("slice_max_samples works", {
  exp <- create_test_exp(c("S1", "S2", "S3", "S4", "S5"), c("V1", "V2"))
  exp$sample_info$score <- c(10, 30, 20, 50, 40)
  
  sliced_exp <- slice_max_samples(exp, order_by = score, n = 2)
...
test_that("slice_max_variables works", {
  exp <- create_test_exp(c("S1", "S2"), c("V1", "V2", "V3", "V4", "V5"))
  exp$var_info$value <- c(5, 15, 10, 25, 20)
  
  sliced_exp <- slice_max_variables(exp, order_by = value, n = 2)
...
test_that("slice_min_samples works", {
  exp <- create_test_exp(c("S1", "S2", "S3", "S4", "S5"), c("V1", "V2"))
  exp$sample_info$score <- c(10, 30, 20, 50, 40)
  
  sliced_exp <- slice_min_samples(exp, order_by = score, n = 2)
...
test_that("slice_min_variables works", {
  exp <- create_test_exp(c("S1", "S2"), c("V1", "V2", "V3", "V4", "V5"))
  exp$var_info$value <- c(5, 15, 10, 25, 20)
  
  sliced_exp <- slice_min_variables(exp, order_by = value, n = 2)
...
test_that("slice functions preserve expression matrix values correctly", {
  ...
  sliced_exp <- slice_samples(exp, 2, 4)
...
test_that("slice functions preserve other experiment components", {
  ...
  sliced_exp <- slice_samples(exp, 1, 3)
...
test_that("slice with proportion works", {
  ...
  sliced_exp <- slice_head_samples(exp, prop = 0.6)
...
test_that("slice with weight_by works", {
  ...
  sliced_exp <- slice_sample_samples(exp, n = 2, weight_by = weight)
...
test_that("slice with ties works", {
  ...
  sliced_exp <- slice_max_samples(exp, order_by = score, n = 2, with_ties = TRUE)
  expect_equal(nrow(sliced_exp$sample_info), 3)  # S4, S2, S3
  
  # With ties = FALSE, should get exactly 2
  sliced_exp <- slice_max_samples(exp, order_by = score, n = 2, with_ties = FALSE)
  expect_equal(nrow(sliced_exp$sample_info), 2)
})
...
test_that("slice with non-existing columns raises an error", {
  ...
  expect_snapshot(slice_max_samples(exp, order_by = bad_col, n = 1), error = TRUE)
  expect_snapshot(slice_min_variables(exp, order_by = bad_col, n = 1), error = TRUE)
})
...
test_that("slice_sample with replace works", {
  ...
  sliced_exp <- slice_sample_samples(exp, n = 5, replace = TRUE)
...
test_that("empty slice operations work", {
  ...
  sliced_exp <- slice_head_samples(exp, n = 0)
  expect_equal(nrow(sliced_exp$sample_info), 0)
  expect_equal(ncol(sliced_exp$expr_mat), 0)
  
  sliced_exp <- slice_tail_variables(exp, n = 0)
  expect_equal(nrow(sliced_exp$var_info), 0)
  expect_equal(nrow(sliced_exp$expr_mat), 0)
}) 