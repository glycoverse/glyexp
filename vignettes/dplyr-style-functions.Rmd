...
- **`_col()` functions**: Work on column data (observation metadata) in `SummarizedExperiment` objects
- **`_row()` functions**: Work on row data (variable metadata) in `SummarizedExperiment` objects

Both variants automatically update the expression matrix to maintain synchronization.
The mental model should be:
"I want to update the row data, so I should use `xxx_row()` functions like regular `dplyr` functions,
...
Each function comes in both `_col()` and `_row()` variants, and all automatically maintain matrix synchronization.

### Core Data Manipulation Functions

| Standard dplyr | Sample Operations | Variable Operations | Description |
|:---|:---|:---|:---|
| `filter()` | `filter_col()` | `filter_row()` | Subset with sync |
| `select()` | `select_col()` | `select_row()` | Choose with protection |
| `arrange()` | `arrange_col()` | `arrange_row()` | Sort with order |
| `mutate()` | `mutate_col()` | `mutate_row()` | Create with consistency |
| `rename()` | `rename_col()` | `rename_row()` | Rename with safety |

### Advanced Slicing Functions

| Standard dplyr | Sample Operations | Variable Operations | Description |
|:---|:---|:---|:---|
| `slice()` | `slice_col()` | `slice_row()` | Position-based selection |
| `slice_head()` | `slice_head_col()` | `slice_head_row()` | Top n with sync |
| `slice_tail()` | `slice_tail_col()` | `slice_tail_row()` | Bottom n with sync |
| `slice_sample()` | `slice_sample_col()` | `slice_sample_row()` | Random with consistency |
| `slice_max()` | `slice_max_col()` | `slice_max_row()` | Highest values with order |
| `slice_min()` | `slice_min_col()` | `slice_min_row()` | Lowest values with order |

### Joining Functions

| Standard dplyr | Sample Operations | Variable Operations | Description |
|:---|:---|:---|:---|
| `left_join()` | `left_join_col()` | `left_join_row()` | Add new columns from another table (left join) |
| `inner_join()` | `inner_join_col()` | `inner_join_row()` | Add new columns from another table (inner join) |
| `semi_join()` | `semi_join_col()` | `semi_join_row()` | Filter rows from another table (semi join) |
| `anti_join()` | `anti_join_col()` | `anti_join_row()` | Filter rows from another table (anti join) |
...
- Using `select_col()` and `select_row()` to keep only needed columns
