...
`GlycomicSE()` and `GlycoproteomicSE()` are structured containers that keep
these data synchronized through the standard `SummarizedExperiment` interface.

> **Lifecycle:** `experiment()` and its manipulation helpers are deprecated.
> The legacy examples below remain for migration of existing code; use the
> `SummarizedExperiment` APIs for new workflows.
